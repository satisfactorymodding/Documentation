...
TODO as of 1.2 actions now have their player mappable settings configured on them and not their contexts.
...
TODO some of these fields don't exist any more
...
Multiple contexts using exactly the same name will be presented under the same heading.
- "M Menu Priority" allows for reordering categories.
  This should always be at least `200` to ensure modded categories end up after base-game ones,
  except "Other" which appears to be hardcoded to always be last.
- "Parent Context" is described in detail below.

TODO better place

- Note that the "Description" field does not seem to appear anywhere in the game UI

- On input actions Player Mappable Context data, the "Display Category" field seems to be ignored
...
TODO add directions about specifically using UFGChildInputMappingContext
...
you may need to add them to the input processing stack by calling (Player Controller instance) > `EnableInput`
for them to actually receive user input events.

[id="RespondInputActionsBlueprint"]
=== Blueprint

An example of this can be found in `ExampleSubsystem` in the ExampleMod.
...
Type the name of the input action asset to create an Event node for responding to it.
For example, if your input action asset is named `IA_TriggerExampleMessage_Play`,
then the node's name in the search will be (Event Node icon) `IA_TriggerExampleMessage_Play`,
and the created node will have the header text `EnhancedInputAction IA_TriggerExampleMessage_Play`

[id="RespondInputActionsCpp"]
=== {cpp}

The https://dev.epicgames.com/documentation/unreal-engine/enhanced-input-in-unreal-engine?application_version=5.6#addinginputlisteners[Unreal documentation]
explains how to handle responding to Input Actions in {cpp},
