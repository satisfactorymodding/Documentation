...
The peculiarity in this case is that hologram position _will not be updated_ while snapped.
...
Whenever it snaps, the Glass Window should be centered on the wall without allowing further movement. So we could write:
...
You need to call by yourself the `SetHologramLocationAndRotation` method like in the example, before returning.
====
