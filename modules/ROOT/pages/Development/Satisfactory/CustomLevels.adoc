...
Uncheck anything that is outside of the the `/ExampleMod/` directory tree.
When you're done, it'll look like this:

.Migration dialog. Sections have been collapsed for the screenshot. All of the children inside the checked items should also be checked (they are by default).
...
.Note the path of your mod reference is present here, not `ExampleMod`
...
Gas Pillars don't visually emit clouds::
  Gas pillars use a special system to control their visual effects. See the floating documentation actors in the example level.
* {blank}
+
...
+
No music is played::
  We aren't sure how to trigger music in custom levels yet.
