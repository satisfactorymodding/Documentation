...
As of 1.2, Satisfactory uses a mod's xref:Development/Satisfactory/GameFeatureDataAsset.adoc[Game Feature Data Asset]
to detect and load icon libraries.

Set up a Primary Asset Types to Scan rule for the `FGIconLibrary` class
and ensure the directory is a parent directory of the folder containing your icon library.
See the linked page for more information on how to set that up.
