...
After specifying fModel's Custom Version and Mapping for Satisfactory, you will/may also want change model export settings.

To do so, you will need to select `Models` on the left side of the fModel settings menu.

From here there are several things you need to do in order to properly export models for use with modeling software such as Blender(shown in image below);
...
* `Level of Detail Format` - Allows you to select how many LOD's of the model to export. 
* `Texture Format` - PNG; This allows you to export textures as PNG.
 
image:ExractingGameFiles/FModelModelSettings.png[FModel Model Export Settings]
