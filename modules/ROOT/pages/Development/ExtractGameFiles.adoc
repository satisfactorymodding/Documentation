...
After the file loads, select `View bank contents` which will open an interactive browser window.

// cspell:ignore HircChunk
If the bnk is linked to any sound files, their id numbers will be displayed in the
`HircChunk > CAkSound > SoundInitialValues > AkMediaInformation > sourceID` field.
Click on the plus buttons and scroll to navigate through the data structure.
Note that multiple sound files can be linked to a single sound event,
each having their own CAkSound entry and sourceID.
