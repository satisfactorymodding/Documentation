...
The next step is to download the mod's source code and put it in your local project.
...
Next, look at the files in the mod source's Content subfolder, if it has one.

* If you see a uasset named exactly the mod's mod reference, or `GameFeatureData`,
  this means the mod is using the xref:Development/Satisfactory/GameFeatureDataAsset.adoc[Game Features system],
  and the plugin folder must be placed in the `Mods/GameFeatures/` directory of your project.
* If the mod has no Content folder, or there is and you don't see the asset,
  the mod can be placed in the base `Mods/` folder of the project.
