...
it will switch to being displayed in a format where 1000 units of the item represents 1 Liter of the fluid.
As such, it's important to make sure that fluids are also assigned the Stack Size of `Fluid`,
allowing it to stack to 50 Liters.
All other stack sizes options result in a decimal quantity less than a 1 liter that is likely to confuse users.
====
