...
First, we'll create the FGItemDescriptor.
...
You can hover over the left side of a field to get an tooltip describing its purpose (pulled from the game headers).
...
M Form::
  ...
  If you decide to make a Liquid or Gas later on, make sure to also assign the stack size of Fluid.
* {blank}
+
M Conveyor Mesh::
...
M Small Icon::
  ...
  When using your own models, you can follow the tutorial
  xref:Development/BeginnersGuide/generating_icons.adoc[here]
  to generate icons that look like the ones in game.
* {blank}
+
M Big Icon::
