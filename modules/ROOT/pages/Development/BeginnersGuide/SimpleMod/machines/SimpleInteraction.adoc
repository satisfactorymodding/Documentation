...
Use it like every other widget in your new widgets and add the content you want in the widget slot "Window Body". Then you can set the title by defining the title variable's default value.
...
you could see everything covered by a gray overlay which effectively blocks you from previewing the UI.
This is expected and doesn't change even with a full asset dump.

To aid you in the development phase, you can create a new Widget and place this inside the Window Body slot:
this way while working on your nested widget, you'll be able to see its content normally.

In alternative, you can open the `Widget_Window_DarkMode` widget in Unreal Editor and hide the gray layer.
...
[NOTE]
====
If the original uasset get updated in the SML repo, you'll have to replace it in your project.
