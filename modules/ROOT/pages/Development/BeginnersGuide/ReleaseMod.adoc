...
Due to changes for dedicated servers, a new checkbox has been added for packaging for xref:CommunityResources/AcronymVault.adoc#_satisfactory_mod_repository_smr_aka_ficsit_app[SMR]. 
This is the `Create merged mod for release`, and will be found under `Linux Server` and 
above `Search Plugin` as well as the `Alpakit Selected!`/`Check All`/`Check None` buttons.
