...
|GameVersion
| Game build version number, used as the (inclusive) minimum version the mod is compatible with.
For example, 273254 is the build number of https://satisfactory.wiki.gg/wiki/Patch_0.8.3.3[Patch 0.8.3.3].
Stored as a string to allow compatibility with potential future numbering format changes.
{blank} +
Alpakit will produce a warning if this field is out of date; clicking the warning will update it for you.
{blank} +
SML checks this field at runtime and will prevent the game from launching (TODO verify) if the game version is below a mod's minimum.
