...
Below is a description of all extra fields SML and SMR utilize:
...
|GameFeature
| (Only present in packaged releases)
Indicates if the mod should be installed to
the GameFeatures subdirectory of the Mods directory (`true`)
or the normal Mods directory (`false`).

Alpakit sets this field for you automatically when building releases,
so this should not be present in your mod source's uplugin.
|===
