...
There are two required builds and two optional builds. Please note that the platforms will show slightly different than the top toolbar method, but they will still work the same. (see image below)

Required: +
Development_Editor - x64 / Development_Editor|Win64 - This is for using the Unreal Engine Editor for developing mods. +
Shipping - x64 / Shipping|Win64 - This is for compiling mods for the client, but also is required for uploading mods to SMR.
...
image::../../../images/BeginnersGuide/BatchBuildingSettings.png[Batch Building, align="center"]
