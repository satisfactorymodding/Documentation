...
. Open the `Unreal Engine` tab in the left sidebar.
...
. Your project should now appear in the Wwise launcher.
  * If multiple projects are listed,
   click the `v` triangle buttons to expand the details
...
. Use the dropdowns to select the version you installed earlier (`{wwise-version-full}`).
  * The version number may have additional numbers on the end of it e.g.
    `{wwise-version-full}.1234` but as long as it starts with `{wwise-version-full}` it should work.
  * If version `{wwise-version-full}` does not appear even after you have selected `All`,
    make sure you got the correct branch of the project,
    and edit the `.uproject` file in a text editor to be sure that `EngineAssociation`
    is set to `{engine-version}-CSS`.
  ** If this version _still_ doesn't show up, contact us for help on the Discord.
. Depending on what the Wwise UI shows:
  .. If you see 2 🟦 blue boxes
     offering "Modify" and "Add Plug-ins" buttons,
     that means it's configured correctly,
     so **continue to the next step**.
  .. If you see a 🟨 yellow warning box
     talking about missed a required component
     with a button to "Modify" the installation, you need to add a component.
     Go ahead and follow what it says for this step only.
  .. If you see a 🟥 red error box
     and Wwise warns you that it could not find an installation
     that means you have not selected the correct version as stated above.
. Scroll down to the "Wwise Project" header
. Click the little triangle dropdown on the right
...
. That's it! Once Wwise shows "Integration completed successfully"
  you have integrated Wwise into your project.
  Close the Wwise launcher as we won't need it any more.
...
* If this option appears, but errors, check out the link:#GenerateVSFiles_Troubleshooting[Troubleshooting section below].
* Otherwise, if the option doesn't appear, you'll need to run a tool from the command line to generate the project files.
+
...
The correct engine version should appear in the picker as `{engine-version}-CSS` and should not be considered a binary build.
...
[id="vs_popups_sign_in"]
==== Sign in to Visual Studio

You do not need to sign in to Visual Studio to use it.

**It is safe to click `Close` or `x` on this message and dismiss it.**
...
You can find more on the xref:Development/UnrealEngine/UnrealLearningResources.adoc[Unreal Learning Resources page].
...
https://dev.epicgames.com/documentation/unreal-engine/unreal-editor-interface?application_version=5.6#maintoolbar[Main Toolbar]
...
https://dev.epicgames.com/documentation/unreal-engine/unreal-editor-interface?application_version=5.6#menubar[Menu Bar].
