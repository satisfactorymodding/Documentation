...
After it completes, you should select `Shipping` in the toolbar and start another build. If you are building for dedicated servers, see link:#_project_compilation_dedicated_server_support_optional[Project Compilation - Dedicated Server Support (Optional)]
...
When you want to test or release your mod, make sure to build the project for `Shipping`, `Shipping Server`, and/or `Linux_Shipping_Server`.
...
If desired, it is possible to build for dedicated server support. However, this is optional and not required to create a mod.
...
It is possible to select targets to build as a group. The advantage is that Visual Studio will perform all the builds requested without requiring user input to select and start each build separately. This is not required, but offered as a means to ease the project build requirements.

It is required to compile the Development_Editor and at least one Shipping build to test/release the mod. Please note that the platforms will show slightly different than the top toolbar method, but they will still work the same. (see image below)

Required: +
Development_Editor - x64 / Development_Editor|Win64 - This is for using the Unreal Engine Editor for developing mods. +

Optional: However, at least one of these is required to test your mod as well as release to (link:https://ficsit.app)[Satisfactory Modding Repository]. +
Shipping - x64 / Shipping|Win64 - Client Support.
