...
The SML adds a neat configuration system that allows you to easily define a config structure based on default values which can then be changed by the user through files in the file system.

These configuration files are then loaded automatically on game boot and override the default values with the values
specified by the user.

The mod can then access the configuration and its values to then change its behaviour depending on this static user input.
...
The registerd mod configurations can get marked as dirty.
...
Then, or when the manager gets told to, it will load the configuration file, parse it and store changes defined in these config files to the loaded config value.

So you need to differentiate between schema and value.
The Scheme works like a class or declaration of how the configuration is structured and what attributes it has.
The value is then basically the actual information which should be structured like described in the schema.
