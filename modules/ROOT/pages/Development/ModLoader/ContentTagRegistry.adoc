...
that manages
https://dev.epicgames.com/documentation/unreal-engine/using-gameplay-tags-in-unreal-engine?application_version=5.6[Gameplay Tag Containers]
...
The base game also utilizes a few tags:
...
* `Recipe.SpacePart` and similar are probably used to identify recipes for the recipe cost multiplier feature
...
and https://dev.epicgames.com/documentation/unreal-engine/using-gameplay-tags-in-unreal-engine?application_version=5.6[the UE documentation].
...
Use `UContentTagRegistry::Get()` to get a reference to the registry.

Use `UContentTagRegistry:GetGameplayTagContainerFor` to get the tag container for a class.

From there, use
https://dev.epicgames.com/documentation/unreal-engine/API/Runtime/GameplayTags/FGameplayTagContainer?application_version=5.6#functions[tag container functions] to query for tags.

[id="VanillaGameplayTagsSystem"]
== Vanilla Functionality

As of 1.2, the base game ships with a basic read-only version of a content tag registry
via the `mGameplayTags` field
...
The base game's system has a few notable limitations compared to SML's:

* It is restricted to the above types
* Only content the mod itself defines can be tagged
* Blueprints can only get these tags, not set them
* Tag modifications can't be made conditionally or at runtime while loading

As such, using SML's system for working with tags is recommended when you need these additional features.

When accessing one of these types, SML will reuse the vanilla gameplay tag container instead of making its own.
