...
When a client connects, the host analyzes what mods the client is connecting with.
If a mod has `RequiredOnRemote` set to true,
`RemoteVersionRange` will be checked to ensure the version is compatible.
The reverse (client checking server) is not currently implemented but may be in the future.
