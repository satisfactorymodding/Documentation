...
User input and key bindings were overhauled in Unreal Engine 5 and Update 8.
Please help us test it out and write docs for the system by trying it out and discussing on the Discord.
...
TODO link the docs page https://docs.unrealengine.com/5.1/en-US/enhanced-input-in-unreal-engine/

mention how you have to copy an existing FGInputMappingContext asset because it doesn't show up in create

mention what fields actually end up displayed in the keybinds menu and what doesn't

mention calling Enable Input on unusual actors so they 

mention having to turn off Context Sensitive to be allowed to create the input action events in ex. a subsystem

TODO examplemod binding still triggers when in interact widgets, probably not good

TODO ask D4rk about handling it for widgets
