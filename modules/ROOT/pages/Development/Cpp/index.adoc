...
You can also open the bootstrapper log found under `<SF installation directory>/pre-launch-debug.log`.
...
The crash will not cause the crash reporter to open, which is a signal that you should then checkout
the pre-launch-debug.log since it will tell you which symbol got called.
