...
You can also open the bootstrapper log you can find under `<SF installation>/pre-launch-debug.log`.
This log will contain information about the special linkage the bootstrapper performs.
If your mod requires to call a function of SF that is not compiled into their binaries,
...
You can still run the game but when your mod calls this function, the game will crash.
The crash will happen without the crash reporter, that means you should then checkout
the pre-launch-debug.log because it will also tell you which symbol got called.
This information is will be very useful to fix the bug so keep it ready when asking on discord.
