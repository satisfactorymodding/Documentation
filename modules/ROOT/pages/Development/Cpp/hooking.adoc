...
[source,cpp]
----
helper->GetContextVariableHelper(); // For reading/writing variables on the blueprint that is being hooked (in the example above, this means member variables of BPW_MapMenu)
helper->GetLocalVariableHelper(); // For reading/writing Input variables of the function, as well as any local variables the function is using for execution (but you have to know their names by decompiling the function)
helper->GetOutVariableHelper(); // For reading/writing Output variables of the function
----

All of these Get* methods have documentation about how to discover variables of the appropriate type.  And all of the helpers' functions have examples of how to access variables, as well as explanation of which functions to use. Use your IDE to inspect them or see SML's BlueprintHookHelper.h
