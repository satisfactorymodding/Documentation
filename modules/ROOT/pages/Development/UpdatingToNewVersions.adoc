...
If the systems a mod works with weren't touched by an update,
the mod could end up working without any changes.
...
== Install the New Clang Toolchain

Check up on the xref:Development/BeginnersGuide/dependencies.adoc#ClangToolchain[Dependencies guide]
to see if you need a new clang toolchain and install if so.
Windows' "Add or remove programs" (Apps & features) can tell you what version you currently have installed
by searching `clang`.
...
Make sure to delete any pre-existing engine installer files you may have in your downloads folder
to avoid your system automatically appending `(1)` to the end,
which will confuse the new installer.
...
https://discord.com/channels/555424930502541343/1506318224982147152[these ones from on the Discord].
...
3. Repeat steps 1-2 until the project builds successfully for Development Editor
