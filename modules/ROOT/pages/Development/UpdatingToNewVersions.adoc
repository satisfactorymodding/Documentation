...
A cached build file from a previous version is likely the cause of this.

You can try to resolve this by adding a duplicate line to that mod's access transformers file.
The line can be deleted again later, but it should trick Visual Studio into rebuilding the access transformers correctly.
