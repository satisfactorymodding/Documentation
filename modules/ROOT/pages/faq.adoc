...
Note that the base game's link:https://satisfactory.wiki.gg/wiki/Creative_Mode[Creative Mode (formerly named Advanced Game Settings, abbreviated AGS)] options do disable achievements.
...
. Modify Steam's update settings for Satisfactory
...
. Open your link:#Files_GameConfig[base game configuration files directory].
Rename the `Config` folder to something else like `Config_NewerVersion`.
Older versions of the game may not be compatible with newer configs,
so by renaming the folder, we force it to create a new set of configs on startup.

// cspell:ignore upipelinecache
. Delete any `upipelinecache` files in the `Saved` folder
(the same folder your `Config` folder lives in).
These are auto-generated by the game as needed,
but newer versions can confuse it.

. Find the manifest ID for the version you want to download.
...
.. Go to the `Depots` section and find the Depot row for the game files.
   ...
   Click on the "ID" number for that row to be taken to the depot info page.
.. Click the `Manifests` section.
   Use the "Filter branch" option
...
.. Find the row that corresponds to when the update you want was released,
   for example, Update 1.1 is `19 December 2025 - 14:00:51 UTC`.
   Copy the manifest ID from this row for usage later.
   For example, Update 1.1's manifest ID is `1858885725837604498`.

. Open the Windows Run dialog.
You can find it in "Start programs" or by using Win+R shortcut.

. Submit this command in the Run dialog:
`steam://open/console`

. After 1-10 seconds, your Steam client should open.
It should take you to a new top-tab called 'Console'.

. Prepare a download_depot command for the version you wish to download in the text box.
The format is: `download_depot appId depotId targetManifestId`.
Satisfactory's appId is `526870`.
For example, the command to download Update 1.1 would be `download_depot 526870 526871 1858885725837604498`

. Make sure you have enough space (~30GB) in **the drive that you installed Steam on**.
...
Once you're sure you have enough space, execute the download_depot command by pressing Enter.

. Steam will now download the depot into a new folder.
Continuing the previous examples, the folder would be
`C:\Program Files (x86)\Steam\steamapps\content\app_526870\depot_526871`.
This will take a while (the whole game is being downloaded again from scratch), and _**there's no progress bar.**_
...
Open the folder mentioned in the command completion message and keep it open.

. Open up your **game installation folder** - not the depot this time.
...
for example, another folder in your library folder called `Satisfactory Backup`.
Do not skip this step, since we will be deleting files from your real game install folder.

. Delete all of the files from your game installation folder EXCEPT for these:
...
+
Reusing these manifest files tricks Steam into thinking that the contents of the folder are that of the latest version,
meaning Steam won't try to update the files before you launch.
This means you can play the game without putting Steam in offline mode.

. Go to the folder mentioned in the download_depot command's output in a previous step.
Move these files into your game install directory.
If prompted to replace files, keep the already existing files.

. Use Satisfactory Mod Manager to download the mods you want to play with.
...
Remember, _we will not provide support for this process_, you're on your own here.
...
Because we swapped out the depot files, Steam thinks there's nothing to update.
...
* In order to revert this process and return to the latest version of the game,
  ...
  and restore the old Config folder you renamed earlier.
...
These steps describe setting up a separate copy of the game that
can't use online functionality like multiplayer.

. First, you'll need a copy of the old version of the game.
Follow the steps link:#PlayOlderVersion[here] to set one up.

. While the old version is still in your Steam/Epic folders, use SMM to add the mods you want.
...
Remember, _we offer no support for this process_ - you're on your own here.

. Copy your entire game install directory to another location on your computer.
Expect it to take up about 30 gigabytes.

. Create a shortcut for the `.exe` file in the game's root directory, usually `FactoryGameSteam.exe`.
One way to do this is to right click on it and select create shortcut.

. Open the shortcut's properties from the right click menu.
...
. You now have a standalone version of the game that will not update or talk to Steam.
...
. Test launching the standalone copy of the game with your shortcut.

. Once you are certain the standalone copy is working, go back into your launcher and 
link:#_how_do_i_verify_my_game_files[verify your game files]
to restore your launcher's copy to the latest update.

. You can now play the old copy of the game by using your shortcut
