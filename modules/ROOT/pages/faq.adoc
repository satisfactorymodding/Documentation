...
* `/FactoryGame/Mods/` (this folder contains your existing mods. If you want to keep them, dont delete the folder)
...
== What if I want to switch between playing a older modded version and the latest update?

Following on from the steps from the last section, simply:

1. Use SMM to add the mods you want. You WILL NOT be able to use SMM to manage mods once this is done, so make sure that you have all the mods you want and that the game runs at a level of stability you're happy with. You may need to switch to an older version of SMM or install mods manualy.

2. Pick a folder or make one in a hard drive that has enough room  for a full copy of the game (around 30 gigabytes). Copy ALL the contents of the satisfactory folder to this folder. 

3. Right click on FactoryGame.exe (or FactoryGameSteam.exe) and select create shortcut. Move that shortcut to your desktop.

4. Right click on that shortcut and select properties. Under the shortcut tab, add '-noSteamClient' after the file path

5. You now have a standalone version of the game that will not update or talk to Steam. This means you wont be able to play online with anyone through this version.

6. Navigate to `%LOCALAPPDATA%\FactoryGame\Saved\SaveGames`. You will need to open the folder named a long stream of numbers (this is your Steam id) and copy any save files you wish to access in the modded version into `%LOCALAPPDATA%\FactoryGame\Saved\SaveGames\common`. This is because this copied version isn't linked to Steam anymore and thus wont be able to see the save games in your personal folder. 

7. Then, just go back into Steam and link:#_how_do_i_verify_my_game_files[use Steam to verify your game files] to restore your steam install to the latest update.

8. You can now play the modded game by clicking on the desktop shortcut you made, while still being able to play the latest version via steam as normal. 

SMM won't let you manage the copied version, since it cant verify if you pirated it or not. This isnt a pirated copy, since you can't do a depot download if you dont own the game, but if you distribute this copy it is piracy and therefore ILEGAL. 
