...
Now just copy the downloaded .smod/.zip file into `<game root>/FactoryGame/Mods`.
...
If you don't have a .smod/.zip of the mod you still can use them.

Just place the mod's plugin folder into `<game root>/FactoryGame/Mods` and start the game with the SML config setting `developmentMode` enabled.
