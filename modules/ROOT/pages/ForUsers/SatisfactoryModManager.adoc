...
* If you download the AppImage version, you may need to mark it as executable (via a GUI or `chmod{non-breaking-space}+x{non-breaking-space}filename`) before running it.

* If you use the standalone version, note that you must have already installed any required packages mentioned on the GitHub releases page.
...
by downloading the MacOS build from the https://github.com/satisfactorymodding/SatisfactoryModManager/releases[GitHub releases page].
...
In addition to the funnel dropdown,
you can use search queries and xref:ForUsers/Tags.adoc[Tag] matching to control what mods display in the list.

Advanced users can use link:https://www.fusejs.io/extended-search.html#operators[fusejs extended search operators]
to further refine their SMM search queries.

- `term` matches mods that contain terms close to `term` (link:https://www.fusejs.io/fuzzy-search.html[fuzzy matching])
- `'term` matches mods that include exactly `term` (not fuzzy)
- `!term` excludes mods that match `term`
- link:https://www.fusejs.io/extended-search.html#operators[And more!]
