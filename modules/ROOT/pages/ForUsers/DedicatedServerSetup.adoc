...
. Skim the below ficsit-cli section to stay informed, then continue reading the link:#ImportProfile[next setup step].
...
. Read the below, then continue reading the link:#ImportProfile[next setup step].
...
to create a xref:ForUsers/SatisfactoryModManager.adoc[Mod Manager] profile
on your own computer for your game client
then either using that same profile when configuring a server via SMM
or xref:ForUsers/SatisfactoryModManager.adoc#_sharing_profiles[exporting the profile]
and importing it in ficsit-cli (they are the same file format).
You can then send the exported profile file to your server members so they can configure their own installs accordingly.
