...
A command line tool created by SirDigby that assists mod authors with installing dependencies and setting up the Starter Project files.

Its source code can be found on link:https://github.com/mklierman/SMEH[SirDigyb's GitHub].

=== Satisfactory Modding Environment Installer (SMEI)

A now-archived command line tool that assisted mod authors with installing dependencies and setting up the Starter Project files.
It has been discontinued due to the lead developer's other time commitments leaving no time to maintain it.

Using link:#SMEH[SMEH] instead is recommended.

Its archived source code can be found
