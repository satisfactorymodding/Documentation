...
Next, from that directory, edit the below the Powershell command to point to your game executable, then run it:
...
Make sure to adjust the command to `FactoryGameSteam` if you are using a Steam copy.
====
...
Normally the dumper would be run with the `-nullrhi` flag to avoid waisting processing power on rendering the game's menu
...
Keep it minimized it as the asset dumper operates and focus on the console window.
...
and bring some them in to replace the ones in your freshly generated copy.
