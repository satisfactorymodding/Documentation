...
`FactoryGameEGS.exe -EpicPortal -NoSteamClient -DumpAllGameAssets -{wj}RootAssetPath=/Game -ExcludePackagePaths=/Game/WwiseAudio -ExcludePackageNames=/Game/Geometry/Meshes/1M_Cube_Chamfer -PackagesPerTick=32 -ExitOnFinish -log -NewConsole -RenderOffscreen -norhithread`
...
The game user interface will never open since we used the `-RenderOffscreen -norhithread` arguments,
...
For comparison, a Content folder with placeholders takes up about 366 MB.
...
https://dev.epicgames.com/documentation/unreal-engine/API/Runtime/Engine/UCommandlet?application_version=5.6[commandlet]
