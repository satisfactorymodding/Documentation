...
Create the machine
------------------
Creating a simple machine is fairly straightforward.
First of all, you'll need to create again a `buildable <../buildable>`_ with descriptor and recipe, but this time use f.e. :download:`this <Mesh_SimpleMachine.fbx>` as the mesh and :download:`this <Icon_SimpleMachine.png>` as icon.
...
We also want to make our machine depend on electricity. For that we need to add a `FGPowerConnectionComponent <../../Concepts/PowerNetwork>`_ at the position of the power connection point of the mesh.
