...
The first child in that row is referred by the ``childs`` attribute.
...
Each of those are represented through an instance of an `UField <UField>`_ . Important to note is that each `UField <UField>`_ points with the ``next`` attribute to next one of the class.
...
Inheritance
-----------
To get the superclass of an `UClass <UClass>`_ just simply follow the ``super`` attribute. It contains the pointer to inherited `UStruct <UStruct>`_.
