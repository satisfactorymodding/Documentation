...
First you need to get the `UProperty`_ of the corresponding `UClass <UClass>`_ your `UObject <UObject>`_ uses, then you just need to call the ``getValue`` method and provide the object (context) itself so the `UProperty`_ know from where it should extract the value from.
...
.. Tip:: To get an Returnvalue-Property for a UFunction, add the ``EPropertyFlag::Prop_RetVal``-Flag to a out param.
...
This function has a template which needs to be a C++ "pointer-to-member" which points to the attribute in your native class.
...
They will look for the last given properties offset, add the used byte count onto the offset and then use that one as the new one.
...
That's why other builder classes want the builder directly and not the build description.